...
const listTransactions = async (req, res, next) => {
  ...
  const { _id } = req.user
  const transactions = await Transaction.find({ owner: _id }, '_id type category sum comment date month year balance owner createdAt', { skip, limit: +limit }).populate('owner', 'email').aggregate(
    [
      { $sort: { createdAt: -1 } }
    ]
  )
